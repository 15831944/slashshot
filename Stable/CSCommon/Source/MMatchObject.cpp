...
void MMatchObject::Tick(unsigned long int nTime)
{
	...
	if (CheckStageListTransfer() == true) {
		...
		if ((MGetServerConfig()->GetServerMode() == MMatchServerMode::MMatchServerMode::MSM_TEST) && (pChannel) && (pChannel->GetChannelType() == MCHANNEL_TYPE_CLAN))
		{
...
bool IsEquipableItem(unsigned long int nItemID, int nPlayerLevel, MMatchSex nPlayerSex)
{
	...
	if (MGetServerConfig()->GetServerMode() != MMatchServerMode::MMatchServerMode::MSM_TEST) {	// EVENT때 레벨제한 없이 장착한다
		// 레벨 제한 조건
