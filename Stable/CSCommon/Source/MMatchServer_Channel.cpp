...
bool MMatchServer::ChannelJoin(const MUID& uidPlayer, const MUID& uidChannel)
{
	...
	if ( MGetServerConfig()->IsUseTicket())
	{
		...
		if ( MGetServerConfig()->GetServerMode() == MMatchServerMode::MSM_NORMAL)	{ // 일반 서버일때
		...
		} else if ( (MGetServerConfig()->GetServerMode() == MMatchServerMode::MMatchServerMode::MSM_CLAN) || (MGetServerConfig()->GetServerMode() == MMatchServerMode::MMatchServerMode::MSM_TEST)) { // 클랜 서버일때
			// 클랜/사설 채널이면 티켓 검사한다.
